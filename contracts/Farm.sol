...
// MasterCRP is the master of CRP. He can distribute CRP and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CRP is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterCRP is Initializable, PausableUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. CRPs to distribute per block.
    ...
    // Amount of MasterCRP wallet balance already allocated to pools
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCRPPerShare = accCRPPerShare.add(
                CRPReward.mul(1e30).div(lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterCRP for CRP allocation.
    ...
    // Withdraw LP tokens from MasterCRP.
    ...
    // helps users compound their CRP rewards in pool 0 in a single ttansaction
    function compound() public whenNotPaused{
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeCRPTransfer(address(this), pending);
            }
